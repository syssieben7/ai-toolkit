...
def call(c, mid, prompt):
    m=next(x for x in c["models"] if x["id"]==mid); b=c["backends"][m["backend"]]
...
def grade(name, text):
    ...
    if not chk.exists(): return None
    return bool(re.search(chk.read_text().strip(), text, re.I|re.S))
def main():
    ...
    r.update(model=a.model, case=a.case, ts=int(time.time()), passed=grade(a.case, r["text"]))
    (ROOT/"results").mkdir(exist_ok=True)
    (ROOT/"results"/f"{r['ts']}_{a.model}_{a.case}.json").write_text(json.dumps(r,indent=2))
    print(f"{a.model:16} {a.case:14} pass={r['passed']!s:5} {r['tok_s']:>6} t/s  {r['wall_s']:>6}s  ${r['cost_usd']}")
